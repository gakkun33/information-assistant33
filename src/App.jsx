        ...
        {activeTab === 'category' && (
          <div className="grid grid-cols-2 gap-3 p-2">
            {categories.map(catName => (
              <div
                key={catName}
                className="border p-3 rounded-lg shadow-sm bg-white flex flex-col items-center justify-center h-28 cursor-pointer hover:bg-gray-50 transition-colors duration-200"
                onClick={() => setSelectedCategory(catName)} // クリック時にカテゴリを選択
              >
                {/* フォルダアイコン */}
                ...
                <h3 className="font-bold text-base text-gray-800 text-center">{catName}</h3>
                <span className="text-xs text-gray-500">{results.filter(r => r.category === catName).length}件</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-3 p-2">
            {sourceTypes.map(source => (
              <div
                key={source.value}
                className="border p-3 rounded-lg shadow-sm bg-white flex flex-col items-center justify-center h-28 cursor-pointer hover:bg-gray-50 transition-colors duration-200"
                onClick={() => setSelectedSource(source.value)} // クリック時に保存元を選択
              >
                {/* フォルダアイコン */}
                <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-gray-400 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                </svg>
                <h3 className="font-bold text-base text-gray-800 text-center">{source.label}</h3>
                <span className="text-xs text-gray-500">{results.filter(r => r.type === source.value).length}件</span>
              </div>
            ))}
          </div>
        )}
